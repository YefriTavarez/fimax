...
class LoanCharges(Document):
    ...
    def before_cancel(self):
        if self.flags.dont_update_gl_entries:
            return "dont_update_gl_entries flag is set"

        self.make_gl_entries(cancel=True)

        frappe.db.sql(f"""
            Delete from `tabGL Entry`
            where voucher_type = "Loan Charges"
    ...
    def update_references(self, cancel=False):
        ...
        if self.reference_type in skipped_list:
            return
    ...
    def validate_reference_name(self):
        ...
        if DocStatus(docstatus) is not DocStatus.SUBMITTED:
            frappe.throw(
                __("Selected {0} is not submitted!".format(self.reference_type)))
    ...
    def make_gl_entries(self, cancel=False, adv_adj=False):
        ...
        income_account = get_company_default(
            loan_doc.company, "default_income_account")
    ...
    def is_elegible_for_deletion(self):
        if self.outstanding_amount == 0 \
                or self.paid_amount > 0:
            return False
        return True


def on_doctype_update():
    if frappe.flags.in_install == "fimax":
        return
